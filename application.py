...
def search():
    ...
    if request.method == 'POST':
        ...
        for result in results:
            return render_template("search.html" , checkedvalue = checkedvalue, SQLquerry = SQLquerry ,id = result.id, title = result.title, author = result.author, isbm = result.isbm, year = result.year)
        if len(results) == 0:
